// Package cli builds the mit command tree on top of the mitnews library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: mitnews.DefaultConfig()}

	root := &cobra.Command{
		Use:   "mit",
		Short: "Browse MIT News research and science stories",
		Long: `mit reads MIT News through its public RSS feeds.
No API key is required. Records are returned as table, JSON, JSONL, CSV, TSV, or URLs.

mit is an independent tool and is not affiliated with MIT or MIT News.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = mitnews.NewClient(a.cfg)
	return nil
}
