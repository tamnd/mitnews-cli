...
describes what users get. Pick a format with `-o`, or let mit choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
mit <command> -o table   # aligned columns for reading
mit <command> -o jsonl   # one JSON object per line, for piping
mit <command> -o json    # a single JSON array
mit <command> -o csv     # spreadsheet friendly
mit <command> -o tsv     # tab-separated
mit <command> -o url     # just the URL column
mit <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
mit <command> --fields id,title,url
```
...
```bash
mit <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
mit <command>            # a table, because this is a terminal
mit <command> | wc -l    # JSONL, because this is a pipe
```
